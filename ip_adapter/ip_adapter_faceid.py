...
class FacePerceiverResampler(torch.nn.Module):
    ...
    def forward(self, latents, x):
        x = self.proj_in(x)
        # print(f'clip:{x.shape}, id_latents:{latents.shape}')
        for attn, ff in self.layers:
            latents = attn(x, latents) + latents
            # print(f'id_latents1:{latents.shape}')
            latents = ff(latents) + latents
            # print(f'id_latents2:{latents.shape}')
        latents = self.proj_out(latents)
        # print(f'id_latents_proj:{latents.shape}')
...
class IPAdapterFaceIDPlus:
    ...
    def get_image_embeds(self, faceid_embeds, face_image, s_scale, shortcut):
        ...
        # print(f'clip_image_embeds:{clip_image_embeds.shape}')
        
        faceid_embeds = faceid_embeds.to(self.device, dtype=self.torch_dtype)
        
        # '''our'''
        # bsz, _ = faceid_embeds.shape
        # uncond_clip_image_embeds = uncond_clip_image_embeds.repeat(bsz, 1, 1)
        # '''our'''
        ...
        # '''our'''
        # _,seq_len,dim = image_prompt_embeds.shape
